...
row1_col1, row1_col2 = st.columns(2)  # 첫 번째 행
row2_col1, row2_col2 = st.columns(2)  # 두 번째 행
...
#######################
# 4. Choropleth 지도 시각화 
with row2_col2:  # 두 번째 행의 두 번째 열
    st.subheader("🗺️ 시도별 사업장 현황 및 업종 분포")
    ...
    map_width = 500  # 지도 가로폭 설정
    map_height = 300  # 지도 세로폭 설정
    map_osm = folium.Map(location=[36.8, 127.5], zoom_start=6)

    if map_option == "시도별 사업장 수":
        ...
        choropleth = folium.Choropleth(
            ...
            fill_opacity=0.7,
            line_opacity=0.2,
            legend_name="시도별 사업장 수"
        )
        choropleth.add_to(map_osm)

    elif map_option == "업종별 시도 분포":
    ...
    # Folium 지도를 Streamlit 레이아웃에 삽입
    folium_static(map_osm, width=map_width, height=map_height)

#######################
# 평균인당고지금액이 높은 5개 사업장 배너
with row0_col1:
    st.markdown("""
                <div>
                    <span style="font-size: 25px">TOP 10 사업장</span>
                    <span style="padding-left: 5px; font-size: 12px; color: #666">
                        \t (기준 : 평균인당고지금액)</span
                    >
                </div>
    ...
    banner_placeholder = st.empty()  # 비어 있는 컨테이너 생성

    # 평균인당고지금액이 높은 5개 데이터 추출
    if "평균인당고지금액" in df.columns and "사업장명" in df.columns:
        ...
        while True:
            for _, row in top_10_businesses.iterrows():
                ...
                time.sleep(2)  # 5초 대기 후 다음 배너로 전환